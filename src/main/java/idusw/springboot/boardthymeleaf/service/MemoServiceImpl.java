...
public class MemoServiceImpl implements MemoService{
    ...
    public List<Memo> readList() {
        List<Memo> result = new ArrayList<>();  //Memo 객체를 원소로 갖는 리스트혀 객체를 생성, 배정

        //테이블로부터 모두 읽어와 객체의 리스트로 변환
        //Entity : Service <-> Repository
        List<MemoEntity> entities =  memoRepository.findAll();  // selset * from a_memo
